...
describe("connect initiation nonce", () => {
  ...
  it("does not let concurrent initialization overwrite the first stored session", async () => {
    ...
    await account.setMcpSessionId(connected.endpoint, first.generation, null, "first-session");
    await account.setMcpSessionId(connected.endpoint, second.generation, null, "second-session");
