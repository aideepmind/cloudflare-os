...
describe("portal detection", () => {
  it("preserves the original one-argument call", () => {
    expect(looksLikePortal([{ name: "ordinary_tool" }])).toBe(false);
    expect(looksLikePortal([{ name: "portal_list_servers" }])).toBe(true);
  });
