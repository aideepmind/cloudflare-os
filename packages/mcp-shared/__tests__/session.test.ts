...
it("calls a tool resolved beyond the initial generated catalog", async () => {
  ...
  const host = {
    ...
    call: async (fn: (client: never, connection: { generation: number }) => Promise<unknown>) => fn({
      callTool: async () => ({ content: [{ type: "text", text: "PROJ-1" }] }),
    } as never, { generation: 1 }),
  } as unknown as McpSessionHost;
...
it("does not dispatch a read after its connection generation changes", async () => {
  ...
  const host = {
    ...
    scope: { serverId: "jira" },
    resolveToolForCall: async () => resolved(entry),
    call: async (fn: (client: never, connection: { generation: number }) => Promise<unknown>) =>
