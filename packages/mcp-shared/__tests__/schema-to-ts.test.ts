...
`);
  ...
  it("always exposes discovery, generic calls, and action results", () => {
    ...
    expect(output).toContain(
      "listTools(options: { search: string }): Promise<McpToolSummary[]>;");
    expect(output).toContain(
      "listTools(options: { name: string }): Promise<McpToolInfo[]>;");
  ...
  it("keeps discovery stable when an upstream tool collides with its method name", () => {
    ...
    expect(output).toContain("listTools(options: { search: string })");
