...
export class McpClient {
  ...
  // Lists tools the server offers, following `nextCursor` pagination to exhaustion. `include`, when
  // present, is applied before the count and byte budgets so an aggregator's unrelated tools cannot
  // crowd the requested server out of a bounded catalog.
  //
  // Filtering before the budgets is also what lets a caller validate a grant naming exact tools
  // against a large aggregator: a requested tool can no longer be pushed out of the result by
  // unrelated ones, so its absence here means it is absent from the endpoint.
  ...
  async #list<T extends { name: string }>(
    ...
    for (let page = 0; page < MAX_TOOL_PAGES; page++) {
      ...
      this.#discoveryTools += body.tools?.length ?? 0;
      if (this.#discoveryBytes > MAX_SCANNED_TOOL_BYTES
          || this.#discoveryTools > MAX_SCANNED_TOOLS) {
        return scanLimit();
      }
