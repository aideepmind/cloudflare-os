...
export type ScopedCatalog = {
  ...
  isPortal: boolean;
  // False means the catalog itself proves any absent tool name does not exist.
  truncated: boolean;
};
...
// A tool definition fetched outside a binding's scoped catalog, remembered for a while.
//
// Exists because an endpoint's catalog may not fit one budget: a broad grant may cover more tools
// than can be described at once, so the ones past the cut are fetched individually on first use.
// Kept here next to `scopedCatalog` so one module decides how long a tool definition may be reused,
// rather than each connector inventing its own answer.
//
// Memory-only, and deliberately not written to the facet's store. The stored catalog carries a
// revision that says when the endpoint changed under a grant; a second persisted copy of a few tools
// would be a second thing to keep in step with it, for a cache that costs one `tools/list` to refill.
//
// Absences are remembered too. Without that, a Gadget naming a tool that does not exist would send
// one full paginated listing to the endpoint per call, which is a cheap way to make this gatekeeper
// hammer a server on an agent's behalf.
export class HydratedTools {
  ...
  // Bounds what a Gadget can make this hold by naming tools; the same order of magnitude as a
  // catalog, since holding more than a catalog's worth of single tools would defeat the point.
